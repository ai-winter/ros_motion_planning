...
class XMLGenerator(object):
    ...
    def writeRobotsXml(self, path):
        ...
        def getRobotArg(name: str, index: int=-1):
            if index == -1:
                e = XMLGenerator.createElement("arg", props={"name": name, \
                    "value": "$(eval arg('robot' + str(arg('robot_number')) + '_" + name + "'))"})
            else:
                e = XMLGenerator.createElement("arg", props={"name": "robot" + str(index + 1) + "_" + name, \
        ...
        include.append(XMLGenerator.createElement("arg", props={"name": "robot", \
            "value": "$(eval arg('robot' + str(arg('robot_number')) + '_type'))"}))
        # planner
        ...
        # pose
        include.append(XMLGenerator.createElement("arg", props={"name": "robot_x", \
            "value": "$(eval arg('robot' + str(arg('robot_number')) + '_x_pos'))"}))
        include.append(XMLGenerator.createElement("arg", props={"name": "robot_y", \
            "value": "$(eval arg('robot' + str(arg('robot_number')) + '_y_pos'))"}))
        include.append(XMLGenerator.createElement("arg", props={"name": "robot_z", \
            "value": "$(eval arg('robot' + str(arg('robot_number')) + '_z_pos'))"}))
        include.append(XMLGenerator.createElement("arg", props={"name": "robot_yaw", \
            "value": "$(eval arg('robot' + str(arg('robot_number')) + '_yaw'))"}))

        # recursive start
        cycle = XMLGenerator.createElement("include", props={"file": "$(find sim_env)/launch/include/robots/start_robots.launch.xml",
            "if": "$(eval arg('robot_number') > 1)"})
    ...
    def writePedestrianWorld(self, path):
        ...
        def createHuman(config, index):
            ...
            # plugin
            plugin = XMLGenerator.createElement("plugin", props={"name": human["name"] + "_plugin", 
