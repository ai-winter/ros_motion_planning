...
#define PW(x) ((x) * (x))
    ...
    void computeGroupForce(Agent &me, const utils::Vector2d &desiredDirection, std::vector<Agent> &agents, Group &group) const;
  };
  ...
  inline void SocialForceModel::computeForces(Agent &me, std::vector<Agent> &agents, Map *map)
  {
    ...
    // NOTE: try generating a smoother trajectory, consider previous force
